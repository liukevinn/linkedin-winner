...
def main() -> None:
    ...
    for jf in json_files:
        ...
        # Preserve player order from config on first file seen
    ...
    for player in sorted_players:
        ...
        avg_str = f"{avg:.1f}"
        table.add_row(_PLACE.get(visual_rank, str(visual_rank)), player, pts_str, str(wins), avg_str)
    ...
    if len(overall_winners) == 1:
        msg = Text(f"Overall Winner: {overall_winners[0]}  ({pts_str} pts over {n_days} day(s))", justify="center")
        panel = Panel(msg, title="All-Time Leader", padding=(1, 4))
    else:
        names = " & ".join(overall_winners)
        msg = Text(f"All-Time Tie: {names}  ({pts_str} pts each over {n_days} day(s))", justify="center")
