...
def _compute_game_ranks(
    ...
    for game, raw_scores in game_results.items():
        ...
        for p in players:
            ...
            if raw == HINT_BAN_SENTINEL:
                game_ranks[p] = ("banned", HINT_BAN_SENTINEL)
            elif p in player_ranks:
...
def _save_daily_results(
    ...
    # txt: human-readable table written via a file-backed Console (no record=True on main console)
    ...
    # json: structured data consumed by overall_standings.py
    ...
    console.print(f"Results saved -> results/{date_str}.txt")
...
def _build_leaderboard_html(
    ...
    for player in sorted_players:
        ...
        patches_raw = game_results.get("patches", {}).get(player)
        if patches_raw == HINT_BAN_SENTINEL:
            patches_raw = "banned: used a hint"
        elif not patches_raw:
            patches_raw = "—"
